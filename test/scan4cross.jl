...
#loco
K4= FlxQTL.kinshipLoco(kinshipLin,X1,4,0.8)
T3,Λ3 = FlxQTL.K2eig(K4,true)


#no loco & no penalization
lod2,b2,es2=FlxQTL.gene1Scan(4,T2,λ2,y,X1;H0_up=true);
@test sum((lod2.< 0.0))==0
lod,b,es=FlxQTL.gene1Scan(T2,λ2,y,X1,Z,4;H0_up=true); #inside function (gene1Scan) check
@test sum((lod.< 0.0))==0
...
#MVLMM
lod3,b3,es3=FlxQTL.gene1Scan(T2,λ2,y,X1,4)
@test sum((lod3.< 0.0))==0
...
#penalization
lod2,b2,es2=FlxQTL.gene1Scan(4,T2,λ2,y,X1;penalize=true,H0_up=true);
@test sum((lod2.< 0.0))==0
# lod,b,es=FlxQTL.gene1Scan(4,T2,λ2,y,X1;H0_up=true);
# @test sum((lod.< 0.0))==0
...
#MVLMM
lod3,b3,es3=FlxQTL.gene1Scan(T2,λ2,y,X1,4;penalize=true)
@test sum((lod3.< 0.0))==0
@test typeof(b3)== Array{Float64,3}
@test isposdef(es3.Vc) 
@test isposdef(es3.Σ)
@test es3.loglik <=0.0
#loco & no penalization
lod1,b1,es01=FlxQTL.gene1Scan(4,T3,Λ3,y,X1,true;H0_up=true);
@test sum((lod1.< 0.0))==0.0
lod0,b0,es00=FlxQTL.gene1Scan(T3,Λ3,y,X1,Z,4,true;H0_up=true);
...
#MVLMM
lod4,b4,es4=FlxQTL.gene1Scan(T3,Λ3,y,X1,4,true)
@test sum(lod4.<0.0)==0
...
#penalization
lod1,b1,es01=FlxQTL.gene1Scan(4,T3,Λ3,y,X1,true;penalize=true,H0_up=true);
@test sum((lod1.< 0.0))==0.0
# lod0,b0,es00=FlxQTL.gene1Scan(4,T3,Λ3,y,X1,Z,true,true;H0_up=true);
# @test sum((lod0.< 0.0))==0.0
...
#MVLMM
lod4,b4,es4=FlxQTL.gene1Scan(T3,Λ3,y,X1,4,true;penalize=true)
@test sum(lod4.<0.0)==0
@test typeof(b4)== Array{Float64,3}
for i=axes(Λ3,2)
    print(@test isposdef(es4[i].Vc))
...
#MVLMM
lod2d0,es2d0=FlxQTL.gene2Scan(T2[:,:,1],λ2[:,1],lnY,X7,3;penalize=true,df_prior=Int64(ceil(1.9m)))
...
#MVLMM
lod2d0,es2d0=FlxQTL.gene2Scan(T2[:,:,1],λ2[:,1],lnY,X7,3)
@test sum(lod2d0.<0.0)==0
@test isposdef(es2d0.Vc)
@test isposdef(es2d0.Σ)
...
#loco & penalization
Lod2d,Es2d=FlxQTL.gene2Scan(3,T2,λ2,lnY,X7,true;penalize=true,df_prior=Int64(ceil(1.9m)));
@test sum(lod2d.<0.0)==0
...
@test sum(lod2d0.<0.0)==0
 ...
 # no penalization
 Lod2d,Es2d=FlxQTL.gene2Scan(3,T2,λ2,lnY,X7,true);
@test sum(lod2d.<0.0)==0
...
#penalization
maxlod, H1per0, cutoff2= FlxQTL.mlmmTest(4,3,K3,lnY,X7,true;pval=[0.1 0.01],df_prior=Int64(ceil(1.9m)))
@test sum(maxlod.<0.0)==0
for l=1:2
println(@test isless(0.0,cutoff2[l]))
end
