...
#     n=size(Y,2); p=size(X,1);
    ...
    for l= 1:nperm
        ...
        LODs,H1par0=marker1Scan(q,kmin,cross,perm_est0,λg,λc,Y2,Xnul_t,X,Z,ν₀,Ψ;ρ=ρ,tol0=tol0,tol1=tol)
...
function permTest(nperm::Int64,cross,Kg,Kc,Y,XX::Markers,Z;pval=[0.05 0.01],m=size(Y,1),df_prior=m+1,
        Prior::Matrix{Float64}=cov(Y,dims=2),Xnul=ones(1,size(Y,2)),ρ=0.001,itol=1e-4,tol0=1e-3,tol=1e-4)
    #permutation without LOCO
       ...
       est0,Xnul_t,Y1,X1,Z1 = geneScan(cross,Tg,Tc,λg,λc,Y,XX,Z;tdata=true,Xnul=Xnul,m=m,
                                       df_prior=df_prior,Prior=Prior,ρ=ρ,itol=itol,tol0=tol0,tol=tol)
       maxLODs, H1par_perm= permutation(nperm,cross,Y1,X1,Z1,est0,λg,λc,Xnul_t,df_prior,Prior;ρ=ρ,tol0=tol0,tol=tol)
